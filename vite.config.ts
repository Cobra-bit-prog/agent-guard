...
// @ts-expect-error JS plugin alongside the TS vite config
import { softAcceptPlugin } from "./scripts/soft-accept-plugin.mjs";
// @ts-expect-error JS plugin alongside the TS vite config
...
/** Claude Connectors OAuth discovery during `vite dev` (Nitro middleware is build-only). */
function oauthDiscoveryPlugin(): Plugin {
  return {
    name: "app-builder:oauth-discovery",
    apply: "serve",
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        try {
...
export default defineConfig(({ command, isPreview }) => ({
  ...
  plugins: [
    ...
    authPopupPlugin(),
    oauthDiscoveryPlugin(),
    // Dev-only /__app-env, read by scripts/check-auth-invariant.mjs.
    appEnvPlugin(),
    // Document routes: 406 on non-HTML Accept before Start's 500 HTML-only check.
    softAcceptPlugin(),
