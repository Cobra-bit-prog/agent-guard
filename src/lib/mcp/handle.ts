...
import { METER_AGENT_LEAD, METER_PACKS_FIRST, STAMP_MERCHANT_COPY, STAMP_TICKET_COPY } from "../meter/pricing.ts";
...
const INSTRUCTIONS =
  `${METER_AGENT_LEAD} Meter meter_* tools: no Authorization / Bearer empty. Human App check/checkout: Bearer API key. Conversion: meter_buy_pass (looks_20) → Base sign_exact → meter_watch({invoice_id, payment}) → token → meter_scan. Agent Meter: meter_pricing / meter_buy_pass / meter_watch / meter_scan / meter_preflight / meter_scan_batch / meter_stamp / meter_verify_stamp with no email — ${METER_PACKS_FIRST} look / looks_20 / addresses_100 / stamp_tx. Then meter_watch, then X-Agent-Pass. meter_buy_pass sku looks_20 (or omit) → invoice with pay_to, base_pay_to, accepts (Base USDC EIP-3009 exact and Solana USDC), amount_usd, amount_base_units, reference, pay_url, invoice_id, watch_url, tool_fields, sign_exact, adapter_snippet, next_steps. Sign USDC on your agent machine. Prefer Base EIP-3009 exact to base_pay_to (CDP/AgentKit; no Solana key). Use sign_exact + adapter_snippet to finish in-process (payMeterPassBase / signTypedData; copy src/adapters/meter-pay-base.ts if you need the full file). Then meter_watch with payment until token. Optional Solana: copy src/adapters/meter-pay.ts. We never take keys. Then meter_scan with pass_token. stamp_tx $0.05: ${STAMP_MERCHANT_COPY} ${STAMP_TICKET_COPY} meter_verify_stamp is public. Human App (separate): call check_transfer before a send (Bearer agent API key). get_pricing is public. You keep the keys.`;
