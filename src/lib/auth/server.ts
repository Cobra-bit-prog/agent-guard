...
function authSecret(): string {
  ...
  if (set) return set;
  const seed = env("VERCEL_GIT_COMMIT_SHA") ?? env("VERCEL_DEPLOYMENT_ID");
  if (seed) {
    return createHash("sha256").update(`agent-guard:${seed}`).digest("hex");
  }
...
export const auth = betterAuth({
  ...
  ...(emailAndPasswordEnabled
    ? {
        emailAndPassword: {
          enabled: true,
          requireEmailVerification: confirmationEmailEnabled(),
        },
      }
    : {}),
  ...
  emailVerification: {
    ...
    sendVerificationEmail: async ({ user, url }) => {
      await sendConfirmationEmail({ to: user.email, url });
    },
  },
