...
Copy `meter-pay.ts`. Default sku is look ($0.10). looks_20 is $0.20. Your agent wallet sends USDC on Solana to pay_to with the 402 reference (extra non-signer account on the transfer). Then watch until the pass token. No Phantom.
...
Default sku is look ($0.10). Pack looks_20 is $0.20. Not pass_1h. Docs: https://agent-control.net/docs#agent-meter
